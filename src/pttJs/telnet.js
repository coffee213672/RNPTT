...
import { b2u, u2b, ansiHalfColorConv, parseWaterball } from './string_util'
...
export function TelnetConnection(socket) {
  this.socket = socket
  this.detail = {}
  // this.socket.addEventListener('onopen', this._onOpen.bind(this))
  DeviceEventEmitter.once('open', this._onOpen.bind(this))
  DeviceEventEmitter.addListener('data', data => {
    this.detail = data || {}
    this._onDataAvailable(this.detail)
  })
...
TelnetConnection.prototype._onDataAvailable = function(e) {
  ...
  // console.log(parseWaterball(b2u(str)))
  while (count > 0) {
    var s = str
    count -= s.length
    var n = s.length
