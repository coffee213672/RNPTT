...
import { b2u_base64 } from '../conv/b2u_table'
import { u2b_base64 } from '../conv/u2b_table'
...
export function u2b(it) {
  ...
  for (var i = 0; i < it.length; ++i) {
    ...
    var hi = lib.u2bArray[2 * pos],
      lo = lib.u2bArray[2 * pos + 1]
    if (hi || lo) data += String.fromCharCode(hi) + String.fromCharCode(lo)
    // Not a big5 char
    else data += '\xFF\xFD'
  }
  return data
...
export function parseReplyText(it) {
  return (
    it.indexOf('▲ 回應至 (F)看板 (M)作者信箱 (B)二者皆是 (Q)取消？[F] ') ===
      0 ||
...
export function parseStatusRow(str) {
  var regex = new RegExp(
    / {2}瀏覽 第 (\d{1,3})(?:\/(\d{1,3}))? 頁 *\( *(\d{1,3})%\) {2}目前顯示: 第 0*(\d+)~0*(\d+) 行 *(?:\(y\)回應)?(?:\(X\/?%\)推文)?(?:\(h\)說明)? *\(←\/?q?\)離開 /g
  )
...
export function parseListRow(str) {
  var regex = new RegExp(
    /\[\d{1,2}\/\d{1,2} +星期. +\d{1,2}:\d{1,2}\] \[ .{3} \] +線上\d+人, 我是\w+ +\[呼叫器\](?:關閉|打開) /g
  )
...
export function ansiHalfColorConv(it) {
  ...
  for (var i = 0; i < indices.length; ++i) {
    ...
    str +=
      it.substring(curInd, preEscInd) +
      '\x00' +
      it.substring(ind + 4, ind + 5) +
      it.substring(preEscInd, ind) +
      'm'
    curInd = ind + 5
  }
...
export function loadResources() {
  lib.b2uArray = new Uint8Array(decode(b2u_base64))
  lib.u2bArray = new Uint8Array(decode(u2b_base64))
}
