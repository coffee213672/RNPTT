...
TermBuf.prototype = {
    ...
    for (var i = 0; i < n; ++i) {
      ...
      switch (ch) {
        ...
        default:
          ...
          ++this.cur_x
          if (ch2.isLeadByte)
            // previous state before this function
  ...
  updateCharAttr: function() {
    ...
    for (var row = 0; row < rows; ++row) {
      ...
      if (needUpdate) {
        ...
        var s = ''
        for (var col = 0; col < cols; ++col) s += line[col].ch
        ...
        if (line.uris) {
          ...
          for (var iuri = 0; iuri < nuris; ++iuri) {
            ...
            var u
            if (this.view.charset != 'UTF-8') u = urlTemp
            //this.conv.convertStringToUTF8(urlTemp, this.view.charset,  true);
            else {
              var str = ''
  ...
  insert: function(param) {
    ...
    if (cur_x + param >= cols) {
      ...
      for (var col = cur_x; col < cols; ++col) line[col].needUpdate = true
    }
    this.changed = true
    this.queueUpdate()
  },
    ...
    if (n >= this.rows)
      ...
      if (up) {
        ...
        while (--n >= 0) {
          ...
          for (var col = 0; col < cols; ++col) line[col].copyFromNewChar()
        }
        for (var i = 0; i < rows - 1 - scrollEnd; ++i) lines.push(lines.shift())
      } else {
        // move lines up
        for (var i = 0; i < scrollStart; ++i) lines.push(lines.shift())
        while (--n >= 0) {
        ...
        for (var i = 0; i < scrollStart; ++i) lines.unshift(lines.pop())
      }
  ...
  queueUpdate: function(directupdate) {
    ...
    if (directupdate) this.timerUpdate = setTimeout(func, 1)
    else this.timerUpdate = setTimeout(func, 30)
  },
  ...
  notify: function(timer) {
    ...
    if (this.changed) {
      ...
      DeviceEventEmitter.emit('viewUpdate')
    }
    ...
    if (color) {
      ...
      for (var col = colStart; col < colEnd - 1; ++col) {
        if (
          isutf8 &&
          text[col].isLeadByte &&
          this.ansiCmp(text[col], text[col + 1])
        )
          output +=
            this.ansiCmp(text[col], text[col + 1]).replace(/m$/g, ';50m') +
            text[col].ch
        else output += text[col].ch + this.ansiCmp(text[col], text[col + 1])
      }
      output +=
        text[colEnd - 1].ch + this.ansiCmp(text[colEnd - 1], TermChar.newChar)
      return isutf8 && charset != 'UTF-8' ? b2u(output) : output
    }

    text = text.slice(colStart, colEnd)
    return text
      .map(function(c, col, line) {
        if (!c.isLeadByte) {
          if (col >= 1 && line[col - 1].isLeadByte) {
            // second byte of DBCS char
            var prevC = line[col - 1]
            var b5 = prevC.ch + c.ch
            if (this.view.charset == 'UTF-8' || b5.length == 1) return b5
            else return b2u(b5)
          } else return c.ch
        }
      })
  ...
  setPageState: function() {
    ...
    if (
      lastRowText.indexOf('請按任意鍵繼續') > 0 ||
      lastRowText.indexOf('請按 空白鍵 繼續') > 0
    ) {
    ...
    if (
      lastRowText.indexOf(
        ' 編輯文章  (^Z/F1)說明 (^P/^G)插入符號/範本 (^X/^Q)離開'
      ) === 0
    ) {
      this.pageState = 6
      return
    ...
    if (this.isUnicolor(0, 0, 29) && this.isUnicolor(0, 60, 70)) {
      ...
      if (
        ...
        this.isUnicolor(2, 0, 70) &&
        !this.isLineEmpty(1) &&
        (this.cur_x < 19 || this.cur_y == 23)
      ) {
        //console.log('pageState = 2 (LIST)');
        this.pageState = 2 // LIST
      }
    } else if (
      this.isUnicolor(23, 28, 53) &&
      this.cur_y == 23 &&
      this.cur_x == 79
    ) {
      //console.log('pageState = 5 (PASS)');
      ...
      case 4: //LIST
        if (trow > 1 && trow < 22) {
          ...
          if (tcol <= 6) {
            ...
            if (trow > 12) this.mouseCursor = 3
            else this.mouseCursor = 2
            this.clearHighlight()
          } else {
      ...
      case 2: //LIST
        if (trow > 2 && trow < 23) {
          ...
          if (tcol < 2)
            ...
            //]
            this.mouseCursor = 9
          else this.mouseCursor = 4
        } else {
          // if ( trow == 23)
          if (tcol < 2) this.mouseCursor = 12
          else if (tcol > 75) this.mouseCursor = 13
          else this.mouseCursor = 5
        }
        break
      ...
      case 3: //READING
        if (trow == 23) {
          if (tcol < 2)
            //]
            this.mouseCursor = 12
          else if (tcol > 75) this.mouseCursor = 14
          else this.mouseCursor = 5
        } else if (trow === 0) {
          if (tcol < 2)
            //=
            ...
            //]
            this.mouseCursor = 9
          else if (tcol < 7) this.mouseCursor = 1
          else this.mouseCursor = 2
        } else if (trow == 1 || trow == 2) {
          if (tcol < 2)
            //[
        ...
        else if (trow < 12) this.mouseCursor = 2
        else this.mouseCursor = 3
        break

      case 1: //MENU
        if (trow > 0 && trow < 23) {
          if (tcol > 7) this.mouseCursor = 7
          else this.mouseCursor = 1
        } else {
          this.mouseCursor = 0
